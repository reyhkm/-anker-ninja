...
const getTransitNodes = (lineA: string, lineB: string) => {
  ...
  // Nambo line logic = Bogor
  const logicalLineA = lineA === 'NAMBO' ? 'BOGOR' : lineA;
  const logicalLineB = lineB === 'NAMBO' ? 'BOGOR' : lineB;

  if (logicalLineA === logicalLineB) {
    if ((lineA === 'NAMBO' && lineB === 'BOGOR') || (lineA === 'BOGOR' && lineB === 'NAMBO')) return ['CTA'];
    return [];
  }

  // --- TRANSIT RULES ---
  // Menuju LOOP SENEN
  if (logicalLineA === 'BOGOR' && logicalLineB === 'LOOP_SENEN') return ['MRI', 'JNG']; // Dari Bogor, turun Manggarai, ke Jatinegara, ganti arah Senen
  if (logicalLineA === 'CIKARANG' && logicalLineB === 'LOOP_SENEN') return ['JNG']; // Titik perpisahan Cikarang Line adalah Jatinegara
  if (logicalLineA === 'PRIOK' && logicalLineB === 'LOOP_SENEN') return ['KPB'];
  if (logicalLineA === 'RANGKAS' && logicalLineB === 'LOOP_SENEN') return ['THB', 'KPB']; // Rangkas -> Tn.Abang -> Kp.Bandan -> Senen
  if (logicalLineA === 'TANGERANG' && logicalLineB === 'LOOP_SENEN') return ['DU', 'KPB'];

  // BOGOR <-> JALUR LAIN
  if (logicalLineA === 'BOGOR' && logicalLineB === 'CIKARANG') return ['MRI'];
  if (logicalLineA === 'BOGOR' && logicalLineB === 'RANGKAS') return ['MRI', 'THB'];
  if (logicalLineA === 'BOGOR' && logicalLineB === 'TANGERANG') return ['MRI', 'DU'];
  if (logicalLineA === 'BOGOR' && logicalLineB === 'PRIOK') return ['JAKK'];

  // CIKARANG <-> JALUR LAIN
  if (logicalLineA === 'CIKARANG' && logicalLineB === 'BOGOR') return ['MRI'];
  if (logicalLineA === 'CIKARANG' && logicalLineB === 'RANGKAS') return ['THB'];
  if (logicalLineA === 'CIKARANG' && logicalLineB === 'TANGERANG') return ['DU'];
  if (logicalLineA === 'CIKARANG' && logicalLineB === 'PRIOK') return ['KPB'];

  // LOOP SENEN (Dari Senen mau ke jalur lain)
  if (logicalLineA === 'LOOP_SENEN' && logicalLineB === 'BOGOR') return ['JNG', 'MRI'];
  if (logicalLineA === 'LOOP_SENEN' && logicalLineB === 'CIKARANG') return ['JNG'];
  if (logicalLineA === 'LOOP_SENEN' && logicalLineB === 'PRIOK') return ['KPB'];
  if (logicalLineA === 'LOOP_SENEN' && logicalLineB === 'RANGKAS') return ['KPB', 'THB'];
  if (logicalLineA === 'LOOP_SENEN' && logicalLineB === 'TANGERANG') return ['KPB', 'DU'];

  // RANGKAS <-> JALUR LAIN
  if (logicalLineA === 'RANGKAS' && logicalLineB === 'BOGOR') return ['THB', 'MRI'];
  if (logicalLineA === 'RANGKAS' && logicalLineB === 'CIKARANG') return ['THB'];
  if (logicalLineA === 'RANGKAS' && logicalLineB === 'TANGERANG') return ['THB', 'DU'];
  if (logicalLineA === 'RANGKAS' && logicalLineB === 'PRIOK') return ['THB', 'KPB'];

  // TANGERANG <-> JALUR LAIN
  if (logicalLineA === 'TANGERANG' && logicalLineB === 'BOGOR') return ['DU', 'MRI'];
  if (logicalLineA === 'TANGERANG' && logicalLineB === 'CIKARANG') return ['DU'];
  if (logicalLineA === 'TANGERANG' && logicalLineB === 'RANGKAS') return ['DU', 'THB'];
  if (logicalLineA === 'TANGERANG' && logicalLineB === 'PRIOK') return ['DU', 'KPB'];

  // PRIOK <-> JALUR LAIN
  if (logicalLineA === 'PRIOK' && logicalLineB === 'BOGOR') return ['JAKK'];
  if (logicalLineA === 'PRIOK' && logicalLineB === 'CIKARANG') return ['KPB'];
  if (logicalLineA === 'PRIOK' && logicalLineB === 'RANGKAS') return ['KPB', 'THB'];
  if (logicalLineA === 'PRIOK' && logicalLineB === 'TANGERANG') return ['KPB', 'DU'];
